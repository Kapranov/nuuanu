...
module Spec.Auth.Kailua.Roundtrip (specs) where

import Auth.Kailua        hiding ( Kailua
                                 , ParseError
                                 , PublicKey
                                 , addBlock
                                 , mkKailua
...
type Roundtrip = ( Kailua Open Verified -> ByteString
                 , PublicKey -> ByteString -> Either ParseError (Kailua OpenOrSealed Verified)
                 )

roundtrip :: Roundtrip -> NonEmpty Block -> Assertion
roundtrip r bs = roundtrip' r $ (Nothing,) <$> bs

roundtrip' :: Roundtrip -> NonEmpty (Maybe SecretKey, Block) -> Assertion
roundtrip' (s,p) i@(authority' :| blocks') = do
  let addBlocks ku kailua = case ku of
        ((Just sk, b):rest) -> addBlocks rest =<< addSignedBlock sk b kailua
        ((Nothing, b):rest) -> addBlocks rest =<< addBlock b kailua
        []                  -> pure kailua
  sk <- generateSecretKey
  let pk = toPublic sk
  init' <- mkKailuaWith (Just 1) sk (snd authority')
...
roundtrip'' :: Bool -> Roundtrip -> NonEmpty (Maybe SecretKey, Block) -> Assertion
roundtrip'' direct (s,p) i@(authority' :| blocks') = do
  let addSignedBlock' :: SecretKey -> Block -> Kailua Open check -> IO (Kailua Open check)
      addSignedBlock' sk bk kailua = do
        if direct
          then
            addSignedBlock sk bk kailua
          else do
            let req = mkThirdPartyBlockReq kailua
            thirdPartyBlock <- either (fail . ("req " <>)) pure $ mkThirdPartyBlock sk req bk
            either (fail . ("apply " <>)) id $ applyThirdPartyBlock kailua thirdPartyBlock
      addBlocks ku kailua = case ku of
        ((Just sk, b):rest) -> addBlocks rest =<< addSignedBlock' sk b kailua
        ((Nothing, b):rest) -> addBlocks rest =<< addBlock b kailua
        []                  -> pure kailua
  sk <- generateSecretKey
...
thirdPartyBlocks r =
  let mkBlocks = do
        ...
        (_sk3, pkThree) <- (id &&& toPublic) <$> generateSecretKey
        pure $ (Nothing, [block|
                   query("authority");
                   right("file1", "read");
                   right("file2", "read");
