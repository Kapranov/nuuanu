...
module Spec.Auth.Bisque.Roundtrip (specs) where

import Auth.Bisque        hiding ( Bisque
                                 , ParseError
                                 , PublicKey
                                 , addBlock
                                 , mkBisque
...
type Roundtrip = ( Bisque Open Verified -> ByteString
                 , PublicKey -> ByteString -> Either ParseError (Bisque OpenOrSealed Verified)
                 )

roundtrip :: Roundtrip -> NonEmpty Block -> Assertion
roundtrip (s,p) i@(authority' :| blocks') = do
  let addBlocks bs bisque = case bs of
        (b:rest) -> addBlocks rest =<< addBlock b bisque
        []       -> pure bisque
  sk <- generateSecretKey
  let pk = toPublic sk
  init' <- mkBisqueWith (Just 1) sk authority'
